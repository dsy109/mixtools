plotly.mixMCMC <- function(
  x , trace.plot = TRUE , summary.plot = FALSE , burnin = 2000, credit.region = 0.95, col.cr = c("red","green"),
    ...
    for (i in 1:dim(theta)[2]){
      ...
      plot.trace <- plot_ly()%>%
        ...
        layout(
          ...
          xaxis = list(title = xlab.trace,
                       tickfont = list(size = xtick.trace.size),
                       titlefont = list(size = xlab.trace.size)),
          yaxis = list(title = ylab.trace,
                       tickfont = list(size = ytick.trace.size),
                       titlefont = list(size = ylab.trace.size))
        )
      print(plot.trace)
    }
  }
  if(is.matrix(mix.object$x) == TRUE && is.null(mix.object$y) == FALSE && summary.plot == TRUE){
    ...
    plot.summary <- plot_ly()%>%
      ...
      layout(
        ...
        xaxis = list(title = xlab.summary,
                     tickfont = list(size = xtick.summary.size),
                     titlefont = list(size = xlab.summary.size)),
        yaxis = list(title = ylab.summary,
                     tickfont = list(size = ytick.summary.size),
                     titlefont = list(size = ylab.summary.size))
      )
    if (length(col.cr) != k){
...
plotly.mixMCMC(x = MH.out , summary.plot = TRUE , col.cr = c("red" , "green"))
